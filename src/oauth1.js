...
module.exports = function(p, callback) {
	...
	var responseReady = function(){
		...
		request(r, null, function(err, res, data, json) {
			...
			// Was this a preflight request
				...
				if ("STORE oauth_token_secret",json.oauth_token_secret) {
					db.storeCreds({
	...
	// The provider has provisioned a temporary token
		...
		if (!token_secret && p.oauth_token in _token_secrets) {
			...
			var haveSecret = function(secretFromDB){
				token_secret = secretFromDB;
				checkSecret();
			}

			db.getOAuthTokenSecret(p.client_id, p.oauth_token,haveSecret);
